...
Do not explain. Do not ask questions. Find the root cause, implement the fix, test it, and commit the changes.
...
- Cookies support has been added.
- EJS remote component works.
- The bot successfully detects YouTube metadata.
- The Video / Audio / Cancel keyboard is displayed.

Problem:

The inline keyboard buttons do nothing when pressed.
...
The callback handlers already exist in src/bot/createBotApplication.ts around lines 203-260.

Tasks:

1. Audit the entire callback_query flow.
2. Verify that callback_data generated by buildKindKeyboard() exactly matches the regex handlers:
   - /^kind:(video|audio)$/
   - /^fmt:(\d+)$/
   - /^cancel:(.+)$/
   - back
   - abort
3. Verify the bot session is preserved after sending the metadata message.
4. Ensure ctx.session.pendingMetadata and ctx.session.pendingUrl are still available when a callback arrives.
5. Verify the session middleware executes before callback handlers.
6. Check whether replyWithPhoto() or reply() loses the session.
7. Check if callback_data exceeds Telegram's 64-byte limit.
8. Verify reply_markup is actually attached to the outgoing message.
9. Check whether another callbackQuery handler consumes the update before these handlers.
10. Verify middleware order inside createBotApplication().
11. Test the complete callback flow locally.
12. Fix the problem so every button works correctly.
...
Ensure every provider reaches the quality selector correctly.
...
Do not stop until the callback buttons work correctly.
